...
function downloadRoutinePdf(routines: ExamRoutine[]) {
  ...
  const a = document.createElement("a"); a.href = url; a.download = "my-exam-routine.txt"; a.click();
            ...
            {grouped[date].sort((a, b) => a.start_time.localeCompare(b.start_time)).map(r => (
              <div key={r.id} className="flex items-center gap-4 p-3 bg-slate-50 rounded-xl">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-500">{r.class_name}{r.section_name ? ` — ${r.section_name}` : ""}{r.teacher_name ? ` • ${r.teacher_name}` : ""}</p>
                </div>
