...
export function AdminExamRoutinePage() {
  ...
  const addRow = () => {
    setRoutines([...routines, { subject_id: "", exam_date: "", start_time: "09:00", end_time: "11:00", room: "" }]);
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
              <option value="">Select exam...</option>
              {exams?.map(e => (
                <option key={e.id} value={e.id}>{e.name} ({e.exam_type.replace("_", " ")})</option>
              ))}
            </select>
          </div>
          <div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold text-slate-700">
              Add Exam Schedule{selectedExamObj ? ` — ${selectedExamObj.name}` : ""}
            </h3>
          ...
          <div className="space-y-3">
            {routines.map((row, idx) => (
              <div key={idx} className="grid grid-cols-2 sm:grid-cols-6 gap-3 items-end p-3 bg-slate-50 rounded-xl">
                <div className="col-span-2">
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  >
                ...
                <div className="flex items-end gap-1">
                  ...
                  {routines.length > 1 && (
                    <button
                      ...
                      className="p-1.5 rounded-lg text-red-500 hover:bg-red-50 transition mb-0.5"
                    >
          ...
          {bulkCreateMut.isError && (
            <p className="text-xs text-red-600 mt-2 text-right">Failed to save. Check for duplicate entries.</p>
          )}
          {bulkCreateMut.isSuccess && (
            <p className="text-xs text-emerald-600 mt-2 text-right">Exam routine saved. Students and teachers have been notified.</p>
          )}
        </div>
      )}
      ...
      {selectedExam && existingRoutines && existingRoutines.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          <h3 className="text-sm font-bold text-slate-700 mb-4">
            Scheduled Exams ({existingRoutines.length})
          </h3>
                ...
                {existingRoutines.map(r => (
                  <tr key={r.id} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="py-2 px-3 text-right">
                      <button
                        onClick={() => deleteRoutineMut.mutate(r.id)}
