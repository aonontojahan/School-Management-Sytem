...
export function AdminExamPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowForm(false);
      setForm({ name: "", exam_type: "MID_TERM", class_id: "", total_marks: "100", passing_marks: "33", start_date: "", end_date: "" });
    },
  });
  ...
  const handleSubmit = () => {
    ...
    const activeYear = academicYears?.[academicYears.length - 1];
  ...
  const handleDelete = (id: number) => {
    if (confirm("Delete this exam? All marks will be lost.")) {
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          ...
          <div className="flex justify-end mt-4">
            <button
              ...
              disabled={!form.name.trim() || !form.class_id || createMutation.isPending}
          ...
          {createMutation.isError && (
            <p className="text-xs text-red-600 mt-2 text-right">Failed to create exam. Please try again.</p>
          )}
        </div>
      )}
                ...
                <div key={exam.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">
                      {cls?.name || "—"} &bull; {exam.total_marks} marks &bull; Pass: {exam.passing_marks}
                    </p>
